...
function AttackTarget:validate(owner, targetObject, targets)
   ...
   return rangeCheck and hasStats
end
...
--- @param stats SRDStatsComponent
--- @param attackdata AttackData
function Attack:calculateAttackModifiers(stats, attackdata)
...
--- @param stats SRDStatsComponent
--- @param attackdata AttackData
function Attack:calculateDamageModifiers(stats, attackdata)
   local statBonus = stats:getStatBonus("STR")
...
function Attack:perform(level)
   ...
   -- cancel crits
   -- add modifiers
   local attackRoll = baseAttackRoll + self:calculateAttackModifiers(ownerStats, attackDataTarget)
   if attackRoll >= targetStats.naturalAC or crit then
      ...
      finalDamage = finalDamage + self:calculateDamageModifiers(ownerStats, attackDataTarget)
