...
function Internal:on(eventName, callback)
   if not self._onCallbacks[eventName] then self._onCallbacks[eventName] = {} end
...
function Internal:onPointer(eventName, callback)
   if not self._onPointerCallbacks[eventName] then self._onPointerCallbacks[eventName] = {} end
...
do
   ...
   function Internal:renderIntoScene(scene, x, y, w, h, depth)
      ...
      if self._draw then self._draw(self._element, x, y, w, h, depth) end
   end
end
