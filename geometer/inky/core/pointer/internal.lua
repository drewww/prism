...
function Internal:setPosition(x, y)
   if self._x == x and self._y == y and self._mode == PointerMode.POSITION then return self end
...
function Internal:setTarget(target)
   if self._target == target then return self end
...
do
   ...
   function Internal:raise(eventName, ...)
      ...
      for i = 1, self._capturedElements:count() do
         ...
         if not self._overlappingElements:has(element) then elements[#elements + 1] = element end
      end
...
function Internal:captureElement(element, shouldCapture)
   if shouldCapture == nil then shouldCapture = true end
