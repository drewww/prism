...
do
   ...
   function Internal:_resolvePointerOverlappingElements(pointer)
      ...
      if mode == PointerMode.POSITION then
         ...
         if pointerX == nil or pointerY == nil then return end
...
function Internal:finishFrame()
   ...
   if #removedElements > 0 then self._elementsChanged = true end
...
function Internal:render(element, x, y, w, h, depth)
   if not self._beganFrame then error("Can't render Element when frame wasn't begun", 3) end

   local elementInternal = element:__getInternal()

   if not elementInternal:isInitialized() then elementInternal:initialize() end
