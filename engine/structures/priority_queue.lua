...
function PriorityQueue:_bubble_down(i)
   local leftChild = 2 * i
...
function PriorityQueue:pop()
   ...
   if not self:is_empty() then self:_bubble_down(1) end

   return highestPriorityElement
end

function PriorityQueue:is_empty() return #self._heap == 0 end
...
assert(not pq:is_empty(), "PriorityQueue should not be empty after one push.")

-- Test multiple push
pq:push("b", 2)
pq:push("c", 1)
assert(pq:size() == 3, "Size should be 3 after three pushes.")
assert(not pq:is_empty(), "PriorityQueue should not be empty after three pushes.")
...
assert(pq:is_empty(), "PriorityQueue should be empty after all elements are popped.")
